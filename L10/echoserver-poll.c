...
int main(int argc, char **argv) {
  ...
  while (!quit) {
    ...
    if(fds[0].revents & POLLIN){
      SA new_client;
      int length = sizeof(new_client);
      char host[MAXLINE], port[MAXLINE];
      Getnameinfo(&new_client, length, host, MAXLINE, port, MAXLINE, NI_NUMERICHOST | NI_NUMERICSERV);
      addclient(fds[0].fd, host, port);
    }
    ...
    while (nready > 0) {
      if(fds[i].revents & POLLIN){
        int bytes_num = clientread(i);
        if(!bytes_num){ // EOF encountered
          delclient(i);
          i--;
        }
        nready--;
