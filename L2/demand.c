...
static void sigsegv_handler(int signum, siginfo_t *info, void *data) {
  ucontext_t *uc = data;
  intptr_t rip;
  rip = uc->uc_mcontext.gregs[14];
  void *addr = (void *) (long)info->si_addr - (long)info->si_addr%pagesize;
  
  if((long)addr >= (long) ADDR_END){
	safe_printf("Address not mapped - terminating!\n");
	exit(139);
  }	 
  safe_printf("Fault at rip=%lx accessing %lx!",rip, (long)info->si_addr);
  
  if(info->si_code == 1){ //SEGV_MAPPER
	safe_printf(" Make page at %lx writable.\n",(long)addr); 
	mmap_page(addr, PROT_WRITE);
  }
  
  if(info->si_code == 2){ //SEGV_ACCERR
 	safe_printf(" Map missing page at %lx\n",(long)addr);
	mprotect_page(addr, PROT_WRITE); 
  }
}
