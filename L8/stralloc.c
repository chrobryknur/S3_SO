...
static void *alloc_block(int8_t *data, uint8_t len) {
  ...
  while (!END_OF_CHUNK(data)) {
    if (BLOCK_USED(data)) {
      ...
      int new_block_size = BLOCK_SIZE(data) - len;
      BLOCK_HEADER(data) = len; // aktualny blok jest zajęty, wiec wartosc dodatnia
      NEXT_BLOCK_HEADER(data) = -new_block_size; // zaznaczamy, ze nastepny blok jest wolny           
      result = CURRENT_PTR(data);      
      break;
      ...
      int new_block_size = BLOCK_SIZE(data) + NEXT_BLOCK_SIZE(data) - len;
      BLOCK_HEADER(data) = len;      
      NEXT_BLOCK_HEADER(data) = -new_block_size;
      result = CURRENT_PTR(data);
      break;
    }
  }
